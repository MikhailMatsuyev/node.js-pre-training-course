...
2. Expose high-level methods:
   ...
   - `toggleStatus(id: number): Promise<Todo>` – cycles the status
     `PENDING → IN_PROGRESS → COMPLETED → PENDING`
   - `setStatus(id: number, status: TodoStatus): Promise<Todo>`
...
4. Add input validation and throw meaningful errors.
   Reject creation when `title` is empty or contains only whitespace.