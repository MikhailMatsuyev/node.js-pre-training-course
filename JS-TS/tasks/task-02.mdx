...
• All helpers must check `source` at runtime and throw a `TypeError` if it is
  `null` or `undefined`. TypeScript types disappear at runtime, so add an
  explicit check like `if (source == null) throw new TypeError(...)` at the
  top of each function — don't rely on the type annotation alone.
...
• `groupBy`'s accumulator needs a type assertion: `const acc = {} as Record<K, T[]>`
  (an empty object literal doesn't satisfy `Record<K, T[]>` for a type parameter `K`).
...
const acc = {} as Record<K, T[]>; // undefined at runtime for a key that never occurred
```
